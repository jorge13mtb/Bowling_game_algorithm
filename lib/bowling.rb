...
#             *****          These are private functions            *****
  ...
  def calculate_the_score_of_this_throw(throw_number)
    ...
    pins_knocked_over_in_this_turn + extra_pins
  end
  ...
  def is_a_strike?(throw_number)
    true if @values_list_of_each_throw[throw_number] == MAX_NUMBER_OF_PINS_IN_THE_GAME
  end


  def is_a_spare?(throw_number)
    true if (@values_list_of_each_throw[throw_number]+@values_list_of_each_throw[throw_number+1]) == MAX_NUMBER_OF_PINS_IN_THE_GAME
  end
